...
export const generateContextImports = (
	...
	return contexts
		.map((context) => {
			...
			const allImports = [...infraImports, ...baseImports].sort((a, b) =>
...
const ops = {
${contexts.map((context) => `\t...Object.fromEntries(Object.entries(${toPascalCase(context.contextName)}Ops).map(([k, v]) => [\`${context.contextName}:\${k}\`, v])),`).join("\n")}
};
`;
};
